...
// write code here 
let split_data = json.data.split(', ')

let count_32 = 0

let key_value_data = ''

for(let i = 0; i < split_data.length; i+=2){
    let key = split_data[i].split('=')[1]
    let age = Number(split_data[i+1].split('=')[1])
    ...
    // prepare key-value data for write into file
    if(i !== 0){
        key_value_data += '\n'
    }
    key_value_data += `${key}:${age}`

}

console.log('Total item exists with 32 age : ', count_32)

fs.writeFile('./npm-tests/output.txt', key_value_data, (err) => {
    // throws an error, you could also catch it here
    if (err) throw err;

    console.log('Key Value Data Saved!');
});